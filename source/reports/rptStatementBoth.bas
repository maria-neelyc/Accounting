...
Begin Report
    ...
    RecSrcDt = Begin
        0xa9d92a934f93e340
    End
    RecordSource ="qryStatementAllBal"
    Caption ="Statement Report"
    OnOpen ="[Event Procedure]"
    DatasheetFontName ="Arial"
    PrtMip = Begin
        0x37020000530300003702000037020000000000005b390000f400000001000000 ,
        ...
        Begin PageHeader
            ...
            Begin
                ...
                Begin TextBox
                    ...
                    TabIndex =8
                    Name ="Text72"
                    ControlSource ="=[FBal]+[FDR]+[FCR]+[FOpenBalDR]-[FOpenBalDR]"
                    Format ="Standard"
                End
                Begin TextBox
                    OldBorderStyle =1
        ...
        Begin Section
            ...
            Begin
                ...
                Begin TextBox
                    ...
                    TabIndex =8
                    Name ="RunBal"
                    ControlSource ="=[RunTotal]+IIf(IsNumeric([Bal]-([DR]-[FCR])),([Bal]-([DR]-[CR])),([OpenBalDR]+["
                        "OpenBalCR]))"
                    Format ="Standard"
                End
                Begin TextBox
                    ...
                    TabIndex =9
                    Name ="Tekst74"
                    ControlSource ="=IIf(IsNumeric([trnsFDebits]),IIf([trnsFDebits]=0,\"\",Abs([trnsFDebits])),\"\")"
                    Format ="Standard"
                End
                Begin TextBox
                    DecimalPlaces =2
                    ...
                    TabIndex =10
                    Name ="Tekst75"
                    ControlSource ="=IIf(IsNumeric([trnsFCredits]),IIf([trnsFCredits]=0,\"\",[trnsFCredits]),\"\")"
                    Format ="Standard"
                End
                Begin Line
                    Left =11955
                ...
                Begin TextBox
                    ...
                    TabIndex =11
                    Name ="FRunBal"
                    ControlSource ="=[FRunTotal]+IIf(IsNumeric([FBal]-([FDR]-[FCR])),([FBal]-([FDR]-[FCR])),([FOpenB"
                        "alDR]+[FOpenBalCR]))"
                    Format ="Standard"
                End
                Begin TextBox
                    ...
                    TabIndex =12
                    Name ="FRunTotal"
                    ControlSource ="=([trnsFDebits]-[trnsFCredits])"
                    Format ="Standard"
                End
            End
        End
        Begin BreakFooter
            ...
            Begin
                ...
                Begin TextBox
                    ...
                    TabIndex =5
                    Name ="Tekst86"
                    ControlSource ="=Sum([trnsFCredits])"
                    Format ="Standard"
                End
                Begin TextBox
                    DecimalPlaces =2
                ...
                Begin TextBox
                    ...
                    TabIndex =7
                    Name ="Tekst88"
                    ControlSource ="=IIf([FBal]<0,Abs([FBal]))"
                    Format ="Standard"
                End
            End
        End
...
Public RunBalance, BalanceBF, FRunBalance, FBalanceBF As Double
Public OldAccNo As String
Public LastEntry, FLastEntry As String
...
Private Sub Detail_Format(Cancel As Integer, FormatCount As Integer)
   ...
   FLastEntry = Format(CDbl(IIf((IsNull(trnsFCredits) Or (trnsFCredits = 0)), IIf(IsNull(trnsFDebits), 0, trnsFDebits), -trnsFCredits)), "##,###,###,##0.00")
   OldAccNo = accNo
...
Private Sub PageHeaderSection_Format(Cancel As Integer, FormatCount As Integer)
   If accNo <> OldAccNo Then
      ...
      lblFBalanceBFDR.Caption = IIf(IsNumeric([FBal] - ([FDR] - [FCR])), IIf([FBal] - ([FDR] - [FCR]) > 0, Format([FBal] - ([FDR] - [FCR]), "##,###,###,##0.00"), ""), IIf(IsNumeric([FOpenBalDR]), IIf([FOpenBalDR] = 0, "", Format([FOpenBalDR], "##,###,###,##0.00")), ""))
      lblFBalanceBFCR.Caption = IIf(IsNumeric([FBal] - ([FDR] - [FCR])), IIf([FBal] - ([FDR] - [FCR]) < 0, Format(Abs([FBal] - ([FDR] - [FCR])), "##,###,###,##0.00"), ""), IIf(IsNumeric([FOpenBalCR]), IIf([FOpenBalCR] = 0, "", Format(Abs([FOpenBalCR]), "##,###,###,##0.00")), ""))
      ...
      Else
         ...
         End If
      End If
      If IsNumeric([FBal] - ([FDR] - [FCR])) Then
         FRunBalance = ([FBal] - ([FDR] - [FCR]))
      Else
         If ([FBal] - ([FDR] - [FCR])) < 0 Then
            If IsNumeric([FOpenBalCR]) Then
                FBalanceBF = [FOpenBalCR]
            Else
                FBalanceBF = 0
            End If
         Else
            If IsNumeric([FOpenBalDR]) Then
                FBalanceBF = [FOpenBalDR]
            Else
                FBalanceBF = 0
            End If
         End If
   ...
   Else
      If Me.Pages > 1 And Me.Page > 1 Then
        ...
        End If
        If FRunBal >= 0 Then
            lblFBalanceBFDR.Caption = Format(CStr(FRunBal - FLastEntry), "##,###,###,##0.00")
            lblFBalanceBFCR.Caption = ""
        Else
            lblFBalanceBFDR.Caption = ""
            lblFBalanceBFCR.Caption = Format(CStr(FRunBal - FLastEntry), "##,###,###,##0.00")
        End If
      End If
   End If
...
If IsNull(Forms.frmReport.cboFromStr1) = True Or Forms.frmReport.cboFromStr1 = "" Then
    Forms.frmReport.cboFromStr1.Value = DLookup("Min(accNo)", "tblAccount") '"0"
End If
If IsNull(Forms.frmReport.cboToStr1) = True Or Forms.frmReport.cboToStr1 = "" Then
    Forms.frmReport.cboToStr1.Value = DLookup("Max(accNo)", "tblAccount") '"9999999999"
End If
...
Select Case Forms.frmReport.grpOptions
    Case 1
        Me.Filter = "(Bal = 0) OR (IsNumeric(Bal)=False) AND accCur>0 "
        Me.FilterOn = True
    Case 2
        Me.Filter = "CR Is Null And DR Is Null  AND accCur>0"
        Me.FilterOn = True
    Case 3
        Me.Filter = "(CR >0 Or DR > 0)  AND accCur)>0"
        Me.FilterOn = True
    Case 4
        Me.Filter = "Bal <> 0  AND accCur>0"
        Me.FilterOn = True
    Case Else
        Me.Filter = "accCur>0"
        Me.FilterOn = True
