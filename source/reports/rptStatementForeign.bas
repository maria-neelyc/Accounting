...
Begin Report
    ...
    Filter ="(CR >0 Or DR > 0)  AND accCur)>0"
        ...
        Begin Section
            ...
            Begin
                ...
                Begin TextBox
                    ...
                    TabIndex =8
                    Name ="RunBal"
                    ControlSource ="=[RunTotal]+IIf(IsNumeric([FBal]-([FDR]-[FCR])),([FBal]-([FDR]-[FCR])),([FOpenBa"
                        "lDR]+[FOpenBalCR]))"
                    Format ="Standard"
                End
            End
...
Private Sub Detail_Format(Cancel As Integer, FormatCount As Integer)
   ...
   LastEntry = Format(CDbl(IIf((IsNull(trnsFCredits) Or (trnsFCredits = 0)), IIf(IsNull(trnsFDebits), 0, trnsFDebits), -trnsFCredits)), "##,###,###,##0.00")
   OldAccNo = accNo
...
Private Sub PageHeaderSection_Format(Cancel As Integer, FormatCount As Integer)
   If accNo <> OldAccNo Then
      lblBalanceBFDR.Caption = IIf(IsNumeric([FBal] - ([FDR] - [FCR])), IIf([FBal] - ([FDR] - [FCR]) > 0, Format([FBal] - ([FDR] - [FCR]), "##,###,###,##0.00"), ""), IIf(IsNumeric([FOpenBalDR]), IIf([FOpenBalDR] = 0, "", Format([FOpenBalDR], "##,###,###,##0.00")), ""))
      lblBalanceBFCR.Caption = IIf(IsNumeric([FBal] - ([FDR] - [FCR])), IIf([FBal] - ([FDR] - [FCR]) < 0, Format(Abs([FBal] - ([FDR] - [FCR])), "##,###,###,##0.00"), ""), IIf(IsNumeric([FOpenBalCR]), IIf([FOpenBalCR] = 0, "", Format(Abs([FOpenBalCR]), "##,###,###,##0.00")), ""))
      If IsNumeric([FBal] - ([FDR] - [FCR])) Then
         RunBalance = ([FBal] - ([FDR] - [FCR]))
      Else
         If ([FBal] - ([FDR] - [FCR])) < 0 Then
            If IsNumeric([FOpenBalCR]) Then
                BalanceBF = [FOpenBalCR]
            Else
                BalanceBF = 0
            End If
         Else
            If IsNumeric([FOpenBalDR]) Then
                BalanceBF = [FOpenBalDR]
            Else
                BalanceBF = 0
            End If
         End If
